...
- `docs/ICD.md` - INT-03 (RC control link), INT-08 (RC transmitter link), RC channel map
- `docs/manual.md` - GR1 switch group, pre-flight safety state
- `params/parameter-change-log.md` - narrative log of intentional parameter changes
- `context/open-items.md`, `docs/build-checklist.md` - open items affecting flight-mode behaviour (CTL-01, AF-01)
...
This matches the switch mapping confirmed against the QGroundControl Flight Modes screen (`assets/flight-modes-config.png`) and documented in `manual.md` Section 3.

Three further modes are reachable outside the GR1 group, each on its own dedicated switch (see `docs/ICD.md` RC channel map):
...
Used during ground functional checks (`manual.md` step 21-22) and as the emergency direct-control fallback. Not used for launch.
...
This is the Believer's default startup mode (GR1 SW2) and the mode used for hand-launch (`manual.md` Section on Assisted Hand Launch). On the V-tail airframe, pitch and yaw commands are both output through the V-tail servos (MAIN 1/2, `docs/ICD.md` INT-02a/INT-02b) via PX4's ruddervator mixing rather than separate elevator and rudder surfaces.

> **Open item:** no yaw authority was observed in Stabilized mode during the 2026-07-10 TMAC session - see CTL-01 in `docs/build-checklist.md`. This must be resolved and re-verified before further flight testing.
...
Requires GPS 1 (M8N, `docs/ICD.md` INT-05) lock; GPS 2 (RTK) is not required for Position mode to function.
...
On the Believer, "Hold" and "Loiter" refer to the same mode - see `manual.md` Section 3 for the naming note. Reachable via GR1 SW6 or directly via CH8 (which overrides GR1). Used as an emergency safe-hold and as the RC-loss failsafe action for a lost data link (Section 6).
...
Reachable via GR1 SW5. Per `manual.md`, reserved for future autonomous missions only - not used for the current hand-launched test flights. Automatic takeoff and landing are tracked as future capability work in `docs/project-roadmap.md`, consistent with `RTL_LAND_DELAY` below currently disabling auto-land.
...
| `RTL_LAND_DELAY` | -1 | Aircraft loiters indefinitely at the destination rather than landing automatically (auto-land is not yet configured - see `docs/project-roadmap.md`) |
...
`RTL_RETURN_ALT` and `RTL_DESCEND_ALT` are both set to 100m, comfortably inside the 120m AGL geofence ceiling (`GF_MAX_VER_DIST`, see `params/parameter-change-log.md`). Reachable via CH10 (overrides GR1), and is also the aircraft's configured RC-loss failsafe action (Section 6).
...
Reachable via CH11 (overrides GR1). Not currently used operationally - the companion computer that would drive it is a future project phase (`docs/project-roadmap.md`).
...
See [PX4: Safety Configuration](https://docs.px4.io/main/en/config/safety.html) for the full failsafe action enum. Battery failsafe thresholds (`BAT_LOW_THR`, `BAT_CRIT_THR`) and geofence breach action (`GF_ACTION`) are documented in `params/parameter-change-log.md`.

## 6. Open Items

- CTL-01 (`docs/build-checklist.md`): no yaw authority observed in Stabilized mode - affects the Stabilized-mode description in Section 4.2 pending investigation.
- AF-01 (`docs/build-checklist.md`): CG out of balance - affects handling in every mode until corrected.
- None of the airspeed, roll/pitch limit, or TECS climb/sink parameters in Sections 4.3-4.4 have been deliberately tuned for the Believer yet (CTL-02, `docs/build-checklist.md`) - values shown are the as-exported PX4 generic fixed-wing airframe defaults.

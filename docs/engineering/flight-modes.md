...
**Offboard is no longer reachable via any switch, as of 2026-09-02.** `RC_MAP_OFFB_SW` was cleared to 0 (unassigned), confirmed via a fresh parameter export - this resolves the SB/SE dual-purpose overlap previously flagged here (flipping elevator rate in flight could no longer trigger Offboard, since PX4 no longer acts on channel 11 for it at all). SE still drives CH11's `mixData` output unchanged in EdgeTX, and separately still selects the elevator tri-rate curve locally (`docs/project/build-checklist.md` CTL-04) - only PX4's interpretation of that channel changed, not the radio configuration itself. Not currently used operationally regardless - the companion computer that would drive Offboard is a future project phase. See Section 4.9.
...
**No longer reachable via any switch, as of 2026-09-02** - `RC_MAP_OFFB_SW` was cleared to 0 (unassigned), confirmed via a fresh parameter export, resolving the dual-purpose overlap against the elevator rate switch previously flagged here (see Section 3). SE still drives CH11's `mixData` output unchanged in EdgeTX and separately still selects the elevator tri-rate curve locally (CTL-04) - only PX4's interpretation of the channel changed. Not currently used operationally regardless - the companion computer that would drive it is a future project phase (`docs/project/project-roadmap.md`) - a new switch assignment (via `RC_MAP_OFFB_SW`) can be made whenever that phase begins.

...
| REQ-PDB-11 | The board shall provide four independent battery-voltage output ports, each terminated in an XT60 connector, each rated for a continuous current of at least 40A. |
| REQ-PDB-12 | The main input connector, and all conductor/trace paths from it, shall be rated for at least the sum of the maximum simultaneous current draw of all connected loads (see Open Items - this cannot be finalised until the use of all four output ports, per REQ-PDB-11, is defined). |
...
| REQ-PDB-31 | The current-sense range shall cover the maximum expected total system current draw (dependent on REQ-PDB-12 / Open Items). |

### 4.5 Electrical - Filtering

| ID | Requirement |
|---|---|
| REQ-PDB-40 | The board shall include bulk/decoupling capacitance at the battery input sufficient to suppress battery-voltage ripple induced by ESC switching. Final sizing is an open item pending ESC selection (see Section 6) - the PM03D's 220µF 63V electrolytic capacitor is the baseline reference point. |
...
| ID | Requirement |
|---|---|
| REQ-PDB-70 | Motor/ESC-side output connectors shall match, or cleanly adapt to, the input connector on the ESCs actually fitted (ESC model currently TBD - `context/open-items.md`). |
...
- Purpose of the two spare distribution ports (REQ-PDB-11) beyond the two motors is not yet defined - affects whether all four ports genuinely need the full 40A rating, and the total current budget for REQ-PDB-12.
- Firm maximum size/weight envelope (REQ-PDB-50) not yet defined - needs a check against available avionics bay clearance.
- Decoupling capacitor sizing (REQ-PDB-40) depends on the switching characteristics of the ESC actually fitted, which is itself unresolved (`context/open-items.md` - ESC model/protocol TBD).
- Current-sense range (REQ-PDB-31) depends on the total system current budget, which depends on the two items above.
- ESC input connector type (REQ-PDB-70) unknown pending ESC model confirmation.

...
- [ ] I2C airspeed sensor (MS4525DO): pull-up resistor configuration not confirmed (port - Pixhawk 6X I2C, JST-GH 4-pin - and address 0x28 are confirmed in `docs/ICD.md` INT-07)
...
- [ ] Recurring airworthiness verification checks (propeller nuts, hatch/nacelle/bolt torque, antenna security) in `docs/build-checklist.md` have no dated inspection record - establish a way to log a dated verification pass (e.g. a per-flight log or a new `docs/test-reports/` entry) and backfill "Last verified" / "Evidence"
